...
export const sendAppointmentBookedNotifications = async ({
  ...
  try {
    ...
    const patient = {
      name: resolveName(patientProfile?.full_name, patientUser?.name, "Patient"),
      email: normalizeEmail(patientProfile?.email || patientUser?.email)
    };
    const doctor = {
      name: resolveName(doctorProfile?.full_name, doctorUser?.name, "Doctor"),
      email: normalizeEmail(doctorProfile?.email || doctorUser?.email)
    };
    ...
    console.error("Could not send appointment booking emails:", error.message);
  }
};
